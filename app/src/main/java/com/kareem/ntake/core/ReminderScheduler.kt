...
import android.app.*;import android.content.Context;import android.content.Intent;import com.kareem.ntake.ReminderReceiver
class ReminderScheduler(private val context:Context){
 fun schedule(noteId:Long,title:String,at:Long):Boolean{if(at<=System.currentTimeMillis())return false;val am=context.getSystemService(AlarmManager::class.java);val pi=pending(noteId,title);return runCatching{if(am.canScheduleExactAlarms())am.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP,at,pi) else am.setAndAllowWhileIdle(AlarmManager.RTC_WAKEUP,at,pi);true}.getOrDefault(false)}
